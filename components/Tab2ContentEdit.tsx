        ...
        if (savedFields) {
            ...
            transformedField.forEach((item) => {
                ...
                if (item?.select) {
                    ...
                    const selected = options.find((opt: any) => opt.selected);
                    if (selected) {
                        initialInputTexts[item.select.name] = selected.value.toString();
                    }
                }
            });
    ...
    const transferDataVisible = (data: TransferField[] = []) => {
        ...
        const renderedComponents = data.map((item, idx) => {
            ...
            switch (type) {
                ...
                case 'select':
                    const selectedOption = componentData.options.find((opt: any) => opt.selected);
                    ...
                    return (
                        <View key={`select-${idx}`} style={[styles.fieldContainer, { marginBottom: 17 }]}>
                            <Text style={[styles.label, { color: '#1C1F37', marginBottom: 10 }]}>{componentData.label}</Text>
                            <Dropdown
